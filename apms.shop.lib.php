...
function apms_order($od_id, $od_status, $od_mk='') {
    ...
    while ($row = sql_fetch_array($result)) {
		...
		if($query) {
		 ...
		 // 주문알림 필드 추가
        global $config;
        apms_response('order', 'order', $row['it_id'], '', '', '', $config['cf_admin'], '', '주문알림', $od_id);							 					   																													
	}
...
function apms_item_rows($arr) {
	...
	$sql_common = "from {$g5['g5_shop_item_table']} where it_use = '1' and it_show_dc = '0' and it_soldout <> '1' $sql_type $sql_ca $sql_ca_ex $sql_pt $sql_term $sql_main $sql_where";
	if($page > 1) {
