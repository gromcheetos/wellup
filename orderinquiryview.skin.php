		...
		<?php for($k=0; $k < count($item[$i]['opt']); $k++) { ?>
			<?php if($k == 0) { ?>
				<tr<?php echo ($i == 0) ? ' class="tr-line"' : '';?>>
					...
					<?php if (defined('PARTNER_GOODS') && $item[$i]['it_id'] == $goods['it_id']) : ?>
					<td colspan="6"><a href="./goods.php?goods_id=<?php echo $goods['link']; ?>"><strong><?php echo $item[$i]['it_name']; ?></strong></a></td>
					<?php else: ?>
...
<?php if ($cancel_price == 0) { // 취소한 내역이 없다면 ?>
	<?php if ($custom_cancel) { ?>
		<div class="print-hide text-center">
                    <form id="cancel" method="post" onsubmit="javascript:openQaWindow(this);" >			
			<input type="hidden" name="od_id"  value="<?php echo $od['od_id']; ?>">
			<input type="hidden" name="token"  value="<?php echo $token; ?>">
			<input type="hidden" name="goods_id"  value="<?php echo $goods_id; ?>">
                        <input type="hidden" id="receiveFromChild" value=""/>
                    <button type="submit" form="cancel" class="btn btn-black btn-sm">주문 취소요청</button>
                    </form>
...
function openQaWindow(){
    var windowObj;
    var settings ='width=700,height=750,location=no,status=no,scrollbars=yes';

    windowObj = window.open('http://wellup.kr/bbs/qawrite.php', 'order_cancel', settings);
    
    windowObj.document.getElementById("childText").value = document.getElementById(od_id).value;
    $("#cancel").submit();
}
</script>
