...
function apms_response($use, $flag, $it_id, $bo_table, $wr_id, $subject, $mb_id, $my_id='', $my_name='', $c_id='') {
    global $g5, $is_admin;
	// 주문알림 필드 추가
    $sql = "alter table $g5[apms_response] add order_cnt int default  0 after qa_cnt";
    sql_query($sql,false);
    $sql = "alter table $g5[apms_response] add od_id varchar(255) default  '' after order_cnt";
    sql_query($sql,false);
	...
	switch($flag) {
		...
		case 'order'	        : $field = 'order'; break; // 주문알림 추가																   
	...
	} else if($use == 'order') { // 주문알림 추가
		if(!$it_id) return;
		if(!$wr_id) $wr_id = 0;
        $od_id = $c_id;
		$where = " mb_id = '$mb_id' and it_id = '$it_id' and wr_id = '$wr_id' and type = '4' and od_id='$od_id' ";
		$set = " it_id = '$it_id', wr_id = '$wr_id', type = '4', od_id='$od_id',  ";
		$purl = G5_ADMIN_URL.'/shop_admin/orderform.php?od_id='.$od_id;     
        $subject = "'[주문] 상품 주문이 있습니다.'";
        $psubj = $subject; 
        $pcont = $psubj;   
...
function apms_response_act($id) {
	...
	if($row['id']) {
		...
		} else if($row['type'] == "4" && $row['od_id']) { // 주문알림 필드 추가
			$url = G5_ADMIN_URL.'/shop_admin/orderform.php?od_id='.$row['od_id'];				 																				
		} else {
			// Comment
